...
void shreceiver_set_receiver(int on){
    ...
    if (on && !is_on) {
        ...
        shreceiver_i2c_write(0x00, 0x07);   /* [TURN_ON] Normal turn on time, tWU = 15ms */
                                            ...
                                            /* [SHDN] Device enabled */
        if ((201==CONFIG_SH_AUDIO_DRIVER_MODEL_NUMBER) || (401==CONFIG_SH_AUDIO_DRIVER_MODEL_NUMBER)) {
            shreceiver_i2c_write(0x01, 0x04);   /* [PLEV2 (B2) PLEV1 (B1)PLEV0 (B0)] VTH(VLIM) = 22VP-P */
        } else {
