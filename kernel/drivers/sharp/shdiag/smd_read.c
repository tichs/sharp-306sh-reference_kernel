...
static int smd_ioctl_get_SBST(unsigned long arg)
{
	int ret = 0;
	sharp_smem_common_type *sharp_smem;
	unsigned int sbstinfo=0;

	sharp_smem = sh_smem_get_common_address();
	if( sharp_smem != 0 )
	{
		sbstinfo = sharp_smem->shdiag_sbstinfo;

		if(copy_to_user((unsigned int __user *)arg, &sbstinfo, sizeof(unsigned int) ) != 0)
		{
			printk("[SH]smd_ioctl_get_sbst: copy_to_user FAILE\n");
			ret = -EFAULT;
		}
	}else{
		printk("[SH]smd_ioctl_get_MSL: get smem_addr FAILE\n");
		return -EFAULT;
	}
...
static int smd_ioctl_get_DDRINFO(unsigned long arg)
{
	int ret = 0;
	sharp_smem_common_type *sharp_smem;
	unsigned char ddrinfo[2]={0,0};

	sharp_smem = sh_smem_get_common_address();
	if( sharp_smem != 0 )
	{
		ddrinfo[0] = sharp_smem->shdiag_ddrinfo[0];
		ddrinfo[1] = sharp_smem->shdiag_ddrinfo[1];

		if(copy_to_user((unsigned char __user *)arg, ddrinfo, (2*sizeof(unsigned char)) ) != 0)
		{
			printk("[SH]smd_ioctl_get_ddrinfo: copy_to_user FAILE\n");
			ret = -EFAULT;
		}
	}else{
		printk("[SH]smd_ioctl_get_ddrinfo: get smem_addr FAILE\n");
		return -EFAULT;
	}
...
static long smd_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case SHDIAG_IOCTL_SET_GPIO_PULL:
		...
		break;
	case SHDIAG_IOCTL_GET_SBST:
		ret = smd_ioctl_get_SBST(arg);
		break;
	case SHDIAG_IOCTL_GET_DDRINFO:
		ret = smd_ioctl_get_DDRINFO(arg);
		break;
	default:
