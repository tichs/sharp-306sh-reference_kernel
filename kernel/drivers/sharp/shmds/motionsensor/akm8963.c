	...
	int32_t *rbuf)
{
	...
	if (ready & FUSION_DATA_READY) {
		...
		input_report_abs(akm->input, ABS_HAT0X, rbuf[8]);
		/* Rotation Vector */
...
//---------------------start
static ssize_t akm_sysfs_selftest_show(
	struct device *dev, struct device_attribute *attr, char *buf)
{
	struct akm_compass_data *akm = dev_get_drvdata(dev);
	int ret, cnt;
	...
	//read sense data
	ret = AKECS_SetMode(akm, AKM_MODE_FUSE_ACCESS);
	if (ret < 0){
		dev_err(&akm->i2c->dev,	"%s: AKECS SetMode ERROR.", __func__);
		return ret;
	}

	asa[0] = AKM_FUSE_1ST_ADDR;
	ret = akm_i2c_rxdata(akm->i2c, asa, 3);
	if (ret < 0){
		dev_err(&akm->i2c->dev,	"%s:I2C read sense data error.", __func__);
		return ret;
	}
	printk("%s:akm fuse  asa[0] = %d, asa[1] = %d, asa[2] = %d\n", __func__, asa[0], asa[1], asa[2]);

	//1.set power down
	ret = AKECS_SetMode(akm, AK8963_MODE_POWERDOWN);
	if (ret < 0){
		dev_err(&akm->i2c->dev,	"%s: AKECS SetMode:power down error.", __func__);
		goto AKM_fail;
	}
	...
	buffer[1] = 0x40;
	ret = akm_i2c_txdata(akm->i2c, buffer, 2);
	if (ret < 0) {
		dev_err(&akm->i2c->dev,	"%s: I2C write error.", __func__);
		goto AKM_fail;		
	}

        //3.Set Self-test Mode
	ret = AKECS_SetMode(akm, AK8963_MODE_SELF_TEST);
	if (ret < 0){
		dev_err(&akm->i2c->dev,	"%s: AKECS SetMode:Self-test error.", __func__);
		goto AKM_fail;
	}

	//4.Check Data Ready or not
	cnt = 10;
	while(cnt > 0) {
	mdelay(10);
	buffer[0] = AKM_REG_STATUS;
	ret = akm_i2c_rxdata(akm->i2c, buffer, 1);
	if (ret < 0) {
		dev_err(&akm->i2c->dev, "I2C write error..");
		goto AKM_fail;			
	}
	 /* Check ST bit */	
	if ((AKM_DRDY_IS_HIGH(buffer[0])))
	...
	buffer[0] = AK8963_REG_HXL;
	ret = akm_i2c_rxdata(akm->i2c, buffer, 6);
	if (ret < 0) {
		dev_err(&akm->i2c->dev, "I2C read error.");
		goto AKM_fail;			
	}
	...
	printk("%s: measurement data x = %d, y = %d, z = %d\n", __func__, x, y, z);
	...
	printk("%s: Hadj x = %d, y = %d, z = %d\n", __func__, x, y, z);
	
	ret = 1;
	if ((x >= -200) && (x <= 200))
		printk("%s: x passed self test, expect -200<=x<=200\n",__func__);
	else {
		printk("%s: x failed self test, expect -200<=x<=200\n",__func__);
		goto AKM_fail;
	}

	if ((y >= -200) && (y <= 200))
		printk("%s: y passed self test, expect -200<=y<=200\n",__func__);
	else {
		printk("%s: y failed self test, expect -200<=x<=200\n",__func__);
		goto AKM_fail;
	}

	if ((z >= -3200) && (z <= -800))
		printk("%s: z passed self test, expect -3200<=z<=-800\n",__func__);
	else {
		printk("%s: z failed self test, expect -3200<=z<=-800\n",__func__);
		goto AKM_fail;
	}
...
AKM_fail:
	...
	return sprintf(buf, "%d %d %d %d %d %d %d\n",
		asa[0], asa[1], asa[2], bias[0], bias[1], bias[2], ret);

}

static ssize_t akm_sysfs_mode_diag_store(
       struct device *dev, struct device_attribute *attr,
       char const *buf, size_t count)
{
       struct akm_compass_data *akm = dev_get_drvdata(dev);
...
static ssize_t akm_sysfs_diag_raw_show(
       struct device *dev, struct device_attribute *attr, char *buf)
{
	struct akm_compass_data *akm = dev_get_drvdata(dev);
	int err;
	signed short x,y,z;
	uint8_t buffer[8];
	unsigned char *stat = &buffer[0];
	unsigned char *stat2 = &buffer[7];
	int status = 0;
	...
	if (status)
		dev_err(&akm->i2c->dev,"%s:line=%d, status=%d\n", __func__, __LINE__, status);

	akm->is_busy = 0;

	if (*stat != 0x00 || *stat2 != 0x00) {
		err = AKECS_SetMode(akm, AKM_MODE_SNG_MEASURE);
		if (err) {
			dev_err(&akm->i2c->dev,"%s: line=%d\n", __func__, __LINE__);
		}
	}
...
//-----------------shart
	__ATTR(selftest,  0440, akm_sysfs_selftest_show, NULL),
        __ATTR(diag_set_mode,  0660, NULL, akm_sysfs_mode_diag_store),
        __ATTR(diag_raw_compass,  0660, akm_sysfs_diag_raw_show,NULL),
//-------------------end
